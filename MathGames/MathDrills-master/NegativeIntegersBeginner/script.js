...
function fetchHighScore() {
  if (typeof Storage !== 'undefined') {
    if (localStorage.highestScoreNegIntBeg) {
      highScore = Number(localStorage.highestScoreNegIntBeg);
...
function gameOver() {
  ...
  if (currentScore > highScore) {
    ...
    localStorage.highestScoreNegIntBeg = currentScore;
    document.querySelector(
      '.streak-message'
...
document.querySelector('.deleteCookies').addEventListener('click', function () {
  highScore = 0;
  localStorage.removeItem('highestScoreNegIntBeg');
