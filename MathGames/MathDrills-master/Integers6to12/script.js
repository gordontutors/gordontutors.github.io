...
function fetchHighScore() {
  if (typeof Storage !== 'undefined') {
    if (localStorage.highestScoreInt6to12) {
      highScore = Number(localStorage.highestScoreInt6to12);
...
function gameOver() {
  ...
  if (currentScore > highScore) {
    ...
    localStorage.highestScoreInt6to12 = currentScore;
    document.querySelector(
      '.streak-message'
...
function randomizeNumbers() {
  //increaseScore();
  const firstRand = parseInt(Math.random() * 7 + 6);
  const secondRand = parseInt(Math.random() * 7 + 6);
...
document.querySelector('.deleteCookies').addEventListener('click', function () {
  highScore = 0;
  localStorage.removeItem('highestScoreInt6to12');
